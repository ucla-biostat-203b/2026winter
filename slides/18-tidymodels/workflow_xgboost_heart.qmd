---
title: "Machine Learning Workflow: XGBoost (Heart Data)"
...
Ensure compatible package versions for Python dependencies.

```{python}
import subprocess
import sys

# Update scikit-learn and xgboost to compatible versions
subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "scikit-learn", "xgboost"])
```
...
We illustrate the typical machine learning workflow for gradient boosting (using XGBoost) using the `Heart` data set. The outcome is `AHD` (`Yes` or `No`).
...
learning_rate_grid = np.logspace(-5, 2, num = 10)
tuned_parameters = {
...
#| code-fold: true
cv_res = pd.DataFrame({
  "learning_rate": np.array(search.cv_results_["param_model__learning_rate"]),
